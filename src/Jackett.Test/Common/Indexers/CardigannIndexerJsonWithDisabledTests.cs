...
{
    [TestFixture]
    public class CardigannIndexerJsonWithDisabledTests
    {
        ...
        [Test]
        public async Task TestCardigannJsonWithDisabledCacheAsync()
        {
            var cacheServiceFactory = _container.Resolve<CacheServiceFactory>();
            var serverConfig =
                new ServerConfig(new RuntimeSettings()) { CacheType = CacheType.Disabled };
